...
\begin{infobox}[colframe=secondarycolor, colback=lightbg]{Glosario de Términos}
    
    \begin{subbox}{Inteligencia Artificial y Computación}
    \begin{itemize}[leftmargin=*, label=\textcolor{secondarycolor}{\textbullet}]
        \item \textbf{Aprendizaje Profundo (Deep Learning):} Rama de la IA basada en redes neuronales de múltiples capas, ideal para extraer patrones complejos en imágenes médicas.
        \item \textbf{Clustering (Agrupamiento):} Técnica de aprendizaje no supervisado que clasifica datos (ej. perfiles de pacientes) basándose en similitudes inherentes sin etiquetas previas.
        \item \textbf{RAG (Generación Aumentada por Recuperación):} Arquitectura que combina un modelo de lenguaje con la búsqueda en bases de datos externas (como bibliografía científica) para generar respuestas precisas y referenciadas.
        \item \textbf{XAI (IA Explicable):} Métodos que hacen transparentes las decisiones de los algoritmos para que los expertos humanos puedan interpretar y confiar en el razonamiento de la máquina.
    \end{itemize}
    \end{subbox}

    \vspace{0.2cm}

    \begin{subbox}{Bioanálisis y Ciencias Renales}
    \begin{itemize}[leftmargin=*, label=\textcolor{secondarycolor}{\textbullet}]
        \item \textbf{Biomarcadores:} Indicadores biológicos medibles objetivamente (en sangre, orina o tejidos) que señalan un estado fisiológico o patológico.
        \item \textbf{Datos Ómicos (Multi-omics):} Conjuntos masivos de información biológica provenientes del estudio integral de genes (Genómica), proteínas (Proteómica) o metabolitos (Metabolómica).
        \item \textbf{ERC (Enfermedad Renal Crónica):} Patología caracterizada por la pérdida progresiva, estructural y funcional, de la capacidad de filtración de los riñones.
        \item \textbf{Histopatología:} Estudio microscópico de los tejidos biológicos para identificar alteraciones morfofisiológicas causadas por enfermedades.
    \end{itemize}
    \end{subbox}
